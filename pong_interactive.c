...
int main() {
  ...
  racket_1 = racket_2 = RACKET_POSITION, ball_x = BALL_POSITION_X,
  ball_y = BALL_POSITION_Y, score_1 = score_2 = 0, dir_x = random_vector(),
  dir_y = random_vector();
  screen_preparation();
  ...
  if (getchar()) {
    ...
    while (1) {
      ...
      if (ball_x < 3) {
        if (racket_1 != ball_y && (racket_1 - 1) != ball_y &&
            (racket_1 + 1) != ball_y && (racket_1 - 2) != ball_y &&
            (racket_1 + 2) != ball_y) {
          score_2 += 1;
          ...
          dir_x = random_vector();
          dir_y = random_vector();
        } else if (racket_1 == ball_y || racket_1 - 1 == ball_y ||
                   racket_1 + 1 == ball_y || racket_1 + 1 == ball_y + dir_y ||
                   racket_1 - 1 == ball_y + dir_y)
          dir_x = -dir_x;
      } else if (ball_x > 78) {
        if (racket_2 != ball_y && (racket_2 - 2) != ball_y &&
            (racket_2 + 2) != ball_y && (racket_2 - 1) != ball_y &&
            (racket_2 + 1) != ball_y) {
          score_1 += 1;
          ball_x = BALL_POSITION_X;
          ball_y = BALL_POSITION_Y;
          dir_x = random_vector();
          dir_y = random_vector();
        } else if (racket_2 == ball_y || racket_2 - 1 == ball_y ||
                   racket_2 + 1 == ball_y || racket_2 + 1 == ball_y + dir_y ||
                   racket_2 - 1 == ball_y + dir_y)
          dir_x = -dir_x;
  ...
  for (int i = 0; i < WIDTH + 2; i++) {
    for (int j = 0; j < LENGTH + 2; j++) {
      if (i == 0 || i == WIDTH + 1 || j == 0 || j == LENGTH + 1) {
        printw("#");
      } else if ((j == (LENGTH / 2) + LENGTH % 2) &&
                 ((ball_x == j && ball_y != i) || ball_x != j)) {
        printw("|");
      } else if (((i == racket_1 || i == racket_1 - 1 || i == racket_1 + 1) &&
                  j == 1) ||
                 ((i == racket_2 || i == racket_2 - 1 || i == racket_2 + 1) &&
                  j == LENGTH)) {
        printw("|");
